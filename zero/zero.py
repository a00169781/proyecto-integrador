...
def run_zero_shot(problem):
    input = problem["input"]
    completion = client.chat.completions.create(
        model=zs_model_params["model"],
        messages=[
            zs_model_params["messages"][0],
            {"role": "user", "content": input},
        ],
    )
...
def run_full_pipeline(original_data):
    data = {}
    data["name"] = "zero_shot results to causal judgement task"
    data["description"] = "This file contains the results of the zero-shot LLM to the causal judgement task."
    ...
    for idx, problem in enumerate(original_data["examples"]):
        ...
        response = run_zero_shot(problem)
...
def main():
    ...
    save_dict_to_drive(data, "./output/zero_shot_results.json")
